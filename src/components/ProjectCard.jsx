...
export default function ProjectCard({ p }) {
  return (
    <div className="rounded-2xl border border-fg/10 p-5 hover:border-accent/70 transition">
      <h3 className="text-lg font-semibold">{p.title}</h3>
      <p className="mt-2 text-sm">{p.summary}</p>
      <div className="mt-3 flex flex-wrap gap-2">
        {p.tags.map(t => <span key={t} className="text-xs px-2 py-0.5 rounded-full bg-fg/12">{t}</span>)}
      </div>
